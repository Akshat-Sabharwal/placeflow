...
export default function CoordinatorDrivesPage() {
  ...
  return <><PageHeader eyebrow="Coordinator" title="Placement drives" description="Create, publish, and move each drive through its placement lifecycle." actions={<Button asChild bg={colors.signal} color={colors.ink}><Link href="/coordinator/drives/new"><Plus size={17} />Create drive</Link></Button>} /><Flex mb="6" bg={colors.surface} border="1px solid" borderColor={colors.line} borderRadius="full" p="1" w="fit-content" maxW="full" overflowX="auto">{(["active", "completed", "cancelled", "all"] as const).map((item) => <Button key={item} size="sm" borderRadius="full" variant="ghost" bg={filter === item ? colors.ink : undefined} color={filter === item ? "white" : colors.muted} onClick={() => setFilter(item)} textTransform="capitalize">{item}</Button>)}</Flex>{drives.length ? <Grid templateColumns={{ base: "1fr", lg: "repeat(2, 1fr)" }} gap="4">{drives.map((drive) => <DriveCard key={drive.id} drive={drive} coordinator />)}</Grid> : <EmptyState title={`No ${filter} drives`} description={filter === "active" ? "Create a draft to begin planning the next placement drive." : "Drives in this state will appear here."} action={filter === "active" ? <Button asChild bg={colors.signal} color={colors.ink}><Link href="/coordinator/drives/new">Create drive</Link></Button> : undefined} />}</>;
}
