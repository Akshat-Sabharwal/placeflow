...
export default function StudentHomePage() {
  ...
  return <><Text color={colors.signalText} fontWeight="800" textTransform="uppercase" letterSpacing=".08em" fontSize="xs">Student workspace</Text><Heading as="h1" mt="2" fontSize={{ base: "3xl", md: "5xl" }} letterSpacing="-.05em">Hi, {name}.</Heading><Text color={colors.muted} mt="3">Here is what needs your attention today.</Text><Grid mt="8" templateColumns={{ base: "1fr", sm: "repeat(3, 1fr)" }} gap="4"><Summary href="/student/drives?filter=open&eligibility=eligible" icon={BriefcaseBusiness} label="Eligible open drives" value={String(open)} /><Summary href="/student/applications?filter=active" icon={LayoutList} label="Active applications" value={String(data?.applications.filter((item) => ["applied", "shortlisted"].includes(item.status)).length ?? 0)} /><Summary href="/student/applications#status-table" icon={FileText} label="Latest status" value={latest ? <StatusBadge status={latest.status} /> : "None yet"} /></Grid><Flex mt="10" justify="space-between" align="center"><Heading as="h2" fontSize="2xl">Open opportunities</Heading><Button asChild variant="ghost" size="sm"><Link href="/student/drives">View all <ArrowRight size={15} /></Link></Button></Flex><Box mt="4">{data?.drives.length ? <Grid templateColumns={{ base: "1fr", lg: "repeat(2, 1fr)" }} gap="4">{data.drives.filter((drive) => drive.status === "published").slice(0, 4).map((drive) => <DriveCard key={drive.id} drive={drive} />)}</Grid> : <EmptyState title="No open drives yet" description="Published drives will appear here automatically." />}</Box></>;
}

function Summary({ href, icon: Icon, label, value }: { href: string; icon: typeof BriefcaseBusiness; label: string; value: React.ReactNode }) {
  return <Link href={href} style={{ display: "block" }}><Box bg={colors.surface} border="1px solid" borderColor={colors.line} borderRadius="16px" p="5" h="full" transition=".16s ease" _hover={{ borderColor: colors.signal, transform: "translateY(-2px)", boxShadow: "var(--shadow-sm)" }}><Flex justify="space-between" align="start"><Box color={colors.signal}><Icon size={20} /></Box><ArrowRight size={16} /></Flex><Text color={colors.muted} fontSize="sm" mt="6">{label}</Text><Box fontSize="2xl" fontWeight="800" mt="1">{value}</Box></Box></Link>;
}
