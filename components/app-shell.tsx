...
export function AppShell({ role, children }: { role: "student" | "coordinator"; children: ReactNode }) {
  ...
  return <Box minH="100vh" bg={colors.paper} color={colors.ink}><ThemeSync />
    ...
    <Flex as="nav" aria-label={`${role} mobile navigation`} display={{ base: "flex", md: "none" }} position="fixed" bottom="0" left="0" right="0" zIndex="30" bg={colors.surface} borderTop="1px solid" borderColor={colors.line} h="72px" overflowX="auto" px="2">{nav.map(({ href, label, icon: Icon }) => { const active = isActive(pathname, href); return <Link key={href} href={href} aria-current={active ? "page" : undefined} style={{ minWidth: 72 }}><Flex h="full" align="center" justify="center" direction="column" gap="1" color={active ? colors.signalDark : colors.muted} fontSize="10px" fontWeight="700"><Icon size={19} />{label}</Flex></Link>; })}</Flex>
  </Box>;
}
