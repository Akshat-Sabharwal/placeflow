...
export function DocumentWorkspace() {
  ...
  const upload = useMutation({
    mutationFn: async () => {
      if (!file || !profile.data?.viewer.userId)
        throw new Error(
  ...
  const remove = useMutation({
    ...
    onError: (error) => {
      setDeleting(null);
      setDurableError(
        error instanceof ApiError && error.code === "DOCUMENT_IN_USE"
  ...
  const openPreview = useMutation({
    mutationFn: async (document: DocumentDTO) => ({
      document,
      url: (await getDocumentUrl(document.id)).signedUrl,
    }),
  ...
  if (documents.isLoading || profile.isLoading)
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Box>
            <Flex color={colors.signal} mb="3">
              <UploadCloud size={26} />
            </Flex>
            <Heading as="h2" fontSize="xl">
              Upload a private document
            </Heading>
            <Text color={colors.muted} mt="2">
              PDF, image, text, Word, Excel, or PowerPoint up to 50 MiB. Files
              stay in your private storage namespace.
            </Text>
          </Box>
            ...
            <Flex gap="3" align="center">
              ...
              <Box>
                ...
                <Text fontSize="sm" color={colors.muted}>
                  {formatBytes(file.size)} · {documentMimeType(file)}
                </Text>
              </Box>
            </Flex>
            <Flex gap="2" align="center">
              <NativeSelect.Root size="sm" w="145px">
                <NativeSelect.Field
        ...
        {uploadError && (
          <Alert.Root status="error" mt="4" borderRadius="12px">
            <Alert.Indicator />
            <Alert.Content>
              <Alert.Title>Upload not completed</Alert.Title>
      ...
      {durableError && (
        <Alert.Root status="warning" mt="5" borderRadius="12px">
          <Alert.Indicator />
          <Alert.Content>
            <Alert.Title>Document kept</Alert.Title>
            <Alert.Description>{durableError}</Alert.Description>
          </Alert.Content>
        </Alert.Root>
      )}
      <Box mt="8">
        ...
        {filteredDocuments.length ? (
          <Flex direction="column" gap="3">
            {filteredDocuments.map((document) => (
              <Flex
                ...
                gap="4"
                transition=".16s ease"
                _hover={{ borderColor: colors.signal, boxShadow: "var(--shadow-sm)" }}
              >
                <button
                  type="button"
                  aria-label={`View ${document.originalName}`}
                  onClick={() => openPreview.mutate(document)}
                  style={{ position: "absolute", inset: 0, zIndex: 1, cursor: "pointer", border: 0, borderRadius: 14, background: "transparent" }}
                />
                <Flex gap="3" align="center" pointerEvents="none">
                  <Box color={colors.signal}>
                    <FileText size={22} />
                  </Box>
                  <Box>
                    <Text fontWeight="700">{document.originalName}</Text>
                    <Text
                      color={colors.muted}
                      fontSize="sm"
                ...
                <Flex gap="2" align="center" zIndex="2">
                  {openPreview.isPending && openPreview.variables?.id === document.id ? <Text color={colors.muted} fontSize="sm">Opening…</Text> : <Flex pointerEvents="none" color={colors.muted} fontSize="sm" gap="1" align="center"><Eye size={15} />Open</Flex>}
                  <Button
                    size="sm"
                    variant="ghost"
                    color={colors.danger}
                    onClick={(event) => { event.stopPropagation(); setDeleting(document); }}
                  >
                    <Trash2 size={16} />
                    Delete
                  </Button>
          ...
          <EmptyState
            ...
            description="Upload resumes, marksheets, certificates, notes, and other placement documents here."
          />
        )}
      </Box>
      <ConfirmDialog
        ...
        description={`Delete ${deleting?.originalName ?? "this document"}. Documents attached to applications will be kept.`}
        ...
        <Portal>
          ...
          <Dialog.Positioner p={{ base: "2", md: "6" }}>
            <Dialog.Content
              ...
              overflow="hidden"
            >
              <Dialog.Header borderBottom="1px solid" borderColor={colors.line}>
                <Box>
                  <Dialog.Title>{previewing?.originalName}</Dialog.Title>
                  <Text color={colors.muted} fontSize="sm">
                    Private signed view · expires shortly
                  </Text>
                </Box>
                <Dialog.CloseTrigger asChild>
                  <Button variant="ghost" aria-label="Close document viewer">
                    <X size={19} />
                  </Button>
                </Dialog.CloseTrigger>
              </Dialog.Header>
              <Dialog.Body p="0" minH="70vh" bg={colors.paperDeep}>
                {previewUrl &&
                  previewing &&
                  (canPreviewDocument(previewing.mimeType) ? (
