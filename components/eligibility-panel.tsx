...
export function EligibilityPanel({ eligible, reasons }: { eligible: boolean; reasons: EligibilityReason[] }) {
  return <Box border="1px solid" borderColor={eligible ? colors.success : colors.warning} bg={eligible ? colors.successSoft : colors.warningSoft} borderRadius="16px" p="5"><Flex align="center" gap="3" color={eligible ? colors.success : colors.warning}>{eligible ? <CheckCircle2 size={22} /> : <CircleX size={22} />}<Heading as="h2" fontSize="lg">{eligible ? "You meet the eligibility rules" : "Not eligible right now"}</Heading></Flex>{reasons.length > 0 && <Flex as="ul" direction="column" gap="2" mt="4" pl="5">{reasons.map((reason) => <Text as="li" key={reason} color={colors.ink}>{reasonCopy[reason]}</Text>)}</Flex>}</Box>;
}
