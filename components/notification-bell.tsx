...
export function NotificationBell() {
  ...
  const pushMutation = useMutation({
    ...
    onSuccess: () => toast.success("Placement notifications are enabled."),
  ...
  return (
    <Box position="relative">
      <Button variant="ghost" size="sm" aria-label={`${unread} unread notifications`} onClick={() => setOpen((value) => !value)} position="relative">
        ...
        {unread > 0 && <Box position="absolute" top="1" right="1" minW="17px" h="17px" borderRadius="full" bg={colors.signalDark} color="white" fontSize="10px" display="grid" placeItems="center">{unread > 9 ? "9+" : unread}</Box>}
      </Button>
      {open && (
        <Box role="dialog" aria-label="Notifications" position="absolute" right="0" top="48px" w={{ base: "calc(100vw - 32px)", sm: "390px" }} maxH="560px" overflowY="auto" bg="white" border="1px solid" borderColor={colors.line} borderRadius="18px" boxShadow="0 18px 55px rgba(23,24,23,.16)" p="4" zIndex="40">
          ...
          {query.isLoading ? <Spinner /> : query.data?.length ? (
            <Flex direction="column" gap="2">
              {query.data.map((item) => (
                <Box key={item.id} p="3" borderRadius="12px" bg={item.readAt ? colors.paper : colors.infoSoft} border="1px solid" borderColor={item.readAt ? colors.line : "#B8CCF1"}>
          ...
          <Box mt="4" pt="4" borderTop="1px solid" borderColor={colors.line}><Text fontWeight="700" fontSize="sm">Never miss a placement update</Text><Text fontSize="xs" color={colors.muted} mt="1">Push is optional. Your in-app updates always remain available.</Text><Button mt="3" size="sm" variant="outline" onClick={() => pushMutation.mutate()} loading={pushMutation.isPending}>Enable notifications</Button></Box>
        </Box>
      )}
    </Box>
  );
