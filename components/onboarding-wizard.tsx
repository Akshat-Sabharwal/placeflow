...
export function OnboardingWizard() {
  ...
  const isProfileRefresh = Boolean(profile.data?.profile.onboardingCompletedAt);
  ...
  async function finish(values: ProfileOutput) {
    ...
    try {
      ...
      toast.success(isProfileRefresh ? "Your active placement profile was updated." : "Your placement profile is ready.");
      router.replace(isProfileRefresh ? "/student/profile" : "/student");
    } catch (error) {
  ...
  return <Box maxW="900px" mx="auto">
    ...
    {step === "source" && <>
      <Heading as="h1" fontSize={{ base: "3xl", md: "5xl" }} letterSpacing="-.05em">{isProfileRefresh ? "Update from a new source document." : "Start with a source document."}</Heading>
      <Text color={colors.muted} mt="3" maxW="680px">{isProfileRefresh ? "Your profile is never edited directly. Upload a current document, review the OCR result, and make it the active profile source seen in placement workflows." : "A PDF or image is required to establish where your profile values came from. It is uploaded to your private document namespace before local extraction."}</Text>
      <Box as="aside" aria-labelledby="profile-source-format" mt="7" bg={colors.infoSoft} color={colors.ink} border="1px solid" borderColor={colors.info} borderRadius="18px" p={{ base: "5", md: "6" }}>
        <Heading id="profile-source-format" as="h2" fontSize="lg">Prepare a consistent profile source</Heading>
        <Box as="ul" mt="3" pl="5" color={colors.muted} fontSize="sm" lineHeight="1.7">
          <li>Upload a PDF, PNG, JPEG, or WebP file up to 50 MiB. For a resume, a text-based, single-column PDF gives the most reliable extraction.</li>
          <li>Label the values PlaceFlow displays: full name, roll number, branch, graduation year, CGPA, current backlogs, and optional LinkedIn and GitHub URLs.</li>
          <li>Use your official branch name without test prefixes, a four-digit graduation year from 2000 to 2100, CGPA from 0 to 10, and a whole-number backlog count from 0 to 99.</li>
          <li>Keep the full name within 120 characters and the roll number and branch within 64 characters. Your profile email comes from the account you signed in with.</li>
        </Box>
      </Box>
      <Box mt="5" bg={colors.surface} border="1px solid" borderColor={colors.line} borderRadius="20px" p={{ base: "6", md: "8" }} transition=".16s ease" _hover={{ borderColor: colors.signal, boxShadow: "var(--shadow-sm)" }}>
        <Box color={colors.signal}><ScanText size={30} /></Box>
        <Heading as="h2" fontSize="2xl" mt="7">Choose a marksheet, student record, or resume</Heading>
        <Text color={colors.muted} mt="2">Images use local OCR. PDFs use embedded text. You review every value before the source becomes active.</Text>
        <Button mt="6" bg={colors.signal} color={colors.onSignal} _hover={{ bg: colors.signalDark }} _active={{ bg: colors.signalDark }} onClick={() => inputRef.current?.click()}><Upload size={17} />Choose PDF or image</Button>
      </Box>
      {message && <Alert.Root status="error" mt="5" borderRadius="14px"><Alert.Indicator /><Alert.Content><Alert.Title>Choose another source</Alert.Title><Alert.Description>{message}</Alert.Description></Alert.Content></Alert.Root>}
      <input ref={inputRef} hidden type="file" accept="image/png,image/jpeg,image/webp,application/pdf,.pdf,.png,.jpg,.jpeg,.webp" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadAndParse(file); }} />
      <Flex mt="6" gap="2" color={colors.muted} fontSize="sm"><ShieldCheck size={18} />Only reviewed profile fields reach the server. Raw OCR text stays in this browser.</Flex>
    </>}
    {step === "extract" && <Box bg={colors.surface} border="1px solid" borderColor={colors.line} borderRadius="20px" p={{ base: "6", md: "9" }} textAlign="center"><Box w="58px" h="58px" mx="auto" display="grid" placeItems="center" borderRadius="full" bg={colors.signalSoft} color={colors.signalText}><FileSearch size={28} /></Box><Heading as="h1" fontSize="2xl" mt="5">{message ? "This source needs another pass" : "Securing and reading locally…"}</Heading><Text data-copyable color={colors.muted} mt="2">{message ?? sourceFile?.name}</Text>{!message && <Progress.Root value={progress} mt="6" colorPalette="orange"><Progress.Track><Progress.Range /></Progress.Track></Progress.Root>}<Flex mt="7" gap="3" justify="center" wrap="wrap">{message && <Button variant="outline" onClick={() => { setStep("source"); setMessage(null); }}><ArrowLeft size={17} />Choose another source</Button>}{message && sourceDocument && <Button bg={colors.signal} color={colors.onSignal} _hover={{ bg: colors.signalDark }} _active={{ bg: colors.signalDark }} onClick={() => { setDraft({}); setStep("review"); }}>Complete from the document manually <ArrowRight size={17} /></Button>}</Flex></Box>}
    {step === "review" && <><Flex justify="space-between" align="start" gap="4" mb="7"><Box><Heading as="h1" fontSize={{ base: "3xl", md: "4xl" }} letterSpacing="-.04em">{isProfileRefresh ? "Review your updated active profile" : "Review and activate your profile"}</Heading><Text color={colors.muted} mt="2">Your source is private and registered. Correct every value before final submission.</Text></Box><Button variant="ghost" onClick={() => setStep("source")}><ArrowLeft size={17} />Start over</Button></Flex>{message && <Alert.Root status="info" mb="5" borderRadius="14px"><Alert.Indicator /><Alert.Content><Alert.Title>Document ready for review</Alert.Title><Alert.Description>{message}</Alert.Description></Alert.Content></Alert.Root>}{submitError && <Alert.Root status="error" mb="5" borderRadius="14px"><Alert.Indicator /><Alert.Content><Alert.Title>Profile not submitted</Alert.Title><Alert.Description>{submitError}</Alert.Description></Alert.Content></Alert.Root>}{rawText && <details style={{ marginBottom: 20 }}><summary style={{ cursor: "pointer", fontWeight: 700 }}>View locally extracted text</summary><Box data-copyable mt="3" maxH="180px" overflowY="auto" whiteSpace="pre-wrap" bg={colors.paperDeep} borderRadius="12px" p="4" color={colors.muted}>{rawText}</Box></details>}<ProfileForm prefill={{ fullName: profile.data?.profile.fullName ?? "", rollNumber: profile.data?.profile.rollNumber ?? "", branch: profile.data?.profile.branch ?? "", graduationYear: profile.data?.profile.graduationYear ?? undefined, cgpa: profile.data?.profile.cgpa ?? undefined, backlogs: profile.data?.profile.backlogs ?? undefined, linkedinUrl: profile.data?.profile.linkedinUrl ?? "", githubUrl: profile.data?.profile.githubUrl ?? "", ...Object.fromEntries(Object.entries(draft).filter(([, value]) => value !== undefined)) }} onReviewSubmit={finish} pending={submitting} submitLabel={isProfileRefresh ? "Activate updated profile" : "Activate profile"} /></>}
  </Box>;
}
