...
export function StudentDrives() {
  ...
  const [filter, setFilter] = useState<Filter>(requestedFilter === "applied" || requestedFilter === "all" ? requestedFilter : "open");
  ...
  return <><Flex gap="3" justify="space-between" direction={{ base: "column", md: "row" }} mb="6"><Flex bg={colors.surface} border="1px solid" borderColor={colors.line} borderRadius="full" p="1" gap="1" aria-label="Drive filters">{(["open", "applied", "all"] as const).map((item) => <Button key={item} size="sm" borderRadius="full" variant={filter === item ? "solid" : "ghost"} bg={filter === item ? colors.ink : undefined} color={filter === item ? colors.paper : colors.muted} onClick={() => setFilter(item)} textTransform="capitalize">{item}</Button>)}</Flex><Box position="relative" w={{ base: "full", md: "290px" }}><Box position="absolute" left="3" top="50%" transform="translateY(-50%)" color={colors.muted}><Search size={17} /></Box><Input value={search} onChange={(event) => setSearch(event.target.value)} pl="10" bg={colors.surface} placeholder="Search company or role" aria-label="Search drives" /></Box></Flex>{eligibleOnly && <Flex mb="4" align="center" gap="2"><Text fontSize="sm" color={colors.success} fontWeight="700">Showing only drives you are eligible for</Text><Button size="xs" variant="ghost" onClick={() => setEligibleOnly(false)}>Show all</Button></Flex>}{query.isRefetchError && <Box mb="5"><RefreshNotice onRetry={() => query.refetch()} /></Box>}{query.isFetching && !query.isLoading && <Flex justify="end" mb="3" color={colors.muted} fontSize="sm" gap="2"><Spinner size="xs" />Updating drives</Flex>}{drives.length ? <Grid templateColumns={{ base: "1fr", lg: "repeat(2, 1fr)" }} gap="4">{drives.map((drive) => <DriveCard key={drive.id} drive={drive} />)}</Grid> : <EmptyState title={search ? "No matching drives" : filter === "applied" ? "You haven't applied to a drive yet" : "No placement drives are open right now"} description={search ? "Try a different company or role." : "New drives will appear here as soon as coordinators publish them."} />}</>;
}
